...
// These tests pin the Lip Gloss v2 behaviour the renderer is built on. They are characterization tests: TML delegates
...
// Style.Width wraps rather than truncating, which is what makes it usable as the text leaf renderer as soon as layout has
...
// Width and Height are the WHOLE block, border included: lipgloss subtracts the border size before it wraps or pads.
...
// Rendering a run of spaces drops the attributes that only decorate a glyph, and Reverse -- which paints the cell
...
// Inherit fills unset fields from a parent style but DELIBERATELY excludes padding and margin. So <Style
...
// Layer coordinates are relative to the parent layer. This is why arrange can emit parent-relative rects and never has
...
// A layer's size is derived from its content, not set on the layer. TML therefore has to render each node to exactly
...
// Hit testing only considers layers carrying an ID, so TML can opt individual elements into mouse routing without
...
// The compositor sorts by each layer's OWN z value, so a parent's z does not create a stacking context for its
func TestZIndexIsGlobalNotScopedToTheParent(t *testing.T) {
	// A child of a low-z parent still paints above a high-z top-level layer when its own z is higher. Nothing scopes it
...
// Sorting is by z alone and is not stable, so equal-z layers have unspecified paint order. The renderer must hand out
...
// Style.Render emits ANSI unconditionally for any colour that is set; downsampling happens later at the writer. Golden
