// Package inspect exposes a running TML program's current frame: what each
// element is, where it landed, what it drew, and what state it is in.
//
// It is the same description whether it comes from a file laid out once or
// from a program mid-run, so a test asserts one shape and a browser renders
// one shape. Nothing here re-implements layout: every number is read off the
// box tree the renderer just painted.
package inspect
...
// Element is one id-bearing element of a frame.
type Element struct {
	ID      string `json:"id"`
	Element string `json:"element"`
	Action  string `json:"action,omitempty"`
	// Focus reports whether the keyboard is on this element. Hover and Held
	// are the pointer's doing.
	Focus bool `json:"focus"`
	Hover bool `json:"hover"`
	Held  bool `json:"held"`
	// Rect is where the element paints, in viewport coordinates. Content is
	// the space inside its margin, border and padding.
	Rect    Rect   `json:"rect"`
	Content Size   `json:"content"`
	Clip    Rect   `json:"clip"`
	Scroll  Scroll `json:"scroll"`
	// Text is everything this element drew, escape sequences removed. Lines is
	// the same text split, which is what a per-line assertion wants.
...
type Options struct {
	// ANSI fills each element's ANSI field. It is off by default because an
	// assertion on styled text is an assertion on the theme.
	ANSI bool
}

// Describe renders box on its own and reports where it landed. state supplies
// the interaction flags, which the box tree does not carry by id.
...
// At returns the id of the innermost element covering the cell, or "" when
// nothing does. It is how a click in the browser preview becomes a selection.
//
// Innermost wins: a click inside a button that sits inside a panel selects the
// button, which is what the pointer was aimed at.
...
// Rectangle is layout's rect under a name this package can range over. It
// exists so covers takes one type whichever rect it is handed.
